...
export default function ProductDetailScreen({ route, navigation }) {
  ...
  const [imageUrl, setImageUrl] = React.useState(
    "https://pgmarket.online/public/images/product/thumb/" + item.thumbnail
  );
  ...
  const getProductFullDetail = () => {
    fetch("https://pgmarket.online/api/getproductfulldetail/" + item.id)
      .then((rest) => rest.json())
      .then((data) => {
  ...
  const addToCart = () => {
    ...
    if (item.status == 0) {
      Alert.alert("Message", t("outOfStockCannotAdd"));
    } else {
    ...
    <ScrollView>
      ...
      >
        ...
        {favorites.some((favorite) => favorite.id === item.id) ? (
          <FavoriteButton onPress={removeFromFavorite} color={colors.accent} />
        ) : (
      ...
      <View style={{ backgroundColor: colors.white }}>
        <View style={{ padding: 10 }}>
          ...
          >
            <ShopCardComponent
              imageURL={
                "https://pgmarket.online/public/images/shop/" + shopInfo.image
              }
          ...
          <View style={{ marginTop: 15, alignItems: "flex-start" }}>
            ...
            {productInfo.video_url && productInfo.video_url != "null" && (
              <TouchableOpacity
                ...
                onPress={() => {
                  Linking.openURL(productInfo.video_url);
                }}
              >
                <Feather name="play" size={24} color={colors.dark} />
                <Text
                  style={{
                    color: colors.dark,
                    textAlign: "center",
