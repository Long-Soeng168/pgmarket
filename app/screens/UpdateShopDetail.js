...
const UpdateShopDetail = ({ navigation }) => {
  ...
  const updateDetails = () => {
    ...
    if (nameError || phoneNumberError || emailError || addressError) {
      return;
    }
    ...
    console.log("Phone Number:", telegramLink);
    ...
    setLoading(true);
    fetch(
      "https://pgmarket.online/api/updateShopProfile/" + user.user.id,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        ...
        <View style={styles.innerContainer}>
          ...
          <View style={{ width: "100%" }}>
            ...
            <TextInput
              ...
              keyboardType="phone-pad"
              value={telegramLink}
              onChangeText={(text) => setTelegramLink(text)}
            />
          </View>
          {phoneNumberError && (
