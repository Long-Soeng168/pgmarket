...
const CheckoutProcess = ({ navigation }) => {
  ...
  const handleRequestOrder = () => {
    ...
    fetch("https://pgmarket.online/api/orderInsert", requestOptions)
      .then((response) => response.json())
      .then((result) => {
        // console.log(result);
        if (!result.error) {
    ...
    return (
      <TouchableOpacity
        style={[
          ...
          {
            backgroundColor:
              item.payment_name === selectedBank ? colors.secondary : "#f0f0f0",
          },
        ]}
  ...
  const handleUploadImage = (invoiceId) => {
    ...
    fetch(
      "https://pgmarket.online/api/uploadTransaction/" + invoiceId,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
      ...
      >
        <ScrollView style={styles.container}>
          ...
          >
            ...
            <View>
              <View>
                {banks.map((item) => (
                  <RenderBankItem item={item} key={item.id.toString()} />
                ))}
                {cashOnDelivery == 1 && (
                  <TouchableOpacity
                    style={[
                      ...
                      {
                        backgroundColor:
                          "On Delivery" === selectedBank
                            ? colors.secondary
                            : "#f0f0f0",
                      },
                    ]}
                    onPress={() => setSelectedBank("On Delivery")}
                  >
                    <View
                      style={[
                        styles.bankLogo,
                    ...
                    <View>
                      <Text style={styles.bankName}>{t("cashOnDelivery")}</Text>
                      <Text>Tap here to pay on delivery</Text>
                    </View>
                  </TouchableOpacity>
                )}
              </View>
        ...
        >
          <TouchableOpacity style={styles.button} onPress={handleConfirm}>
            <Text style={styles.buttonText}>{t("checkouted")}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </>
